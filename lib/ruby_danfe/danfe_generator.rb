  ...
  class DanfeGenerator
    ...
    def render_emitente
      ...
      @pdf.ibox 3.92, emitente_width, emitente_left, 2.54, '',
        ...
        @xml['enderEmit/fone'] + " " + @xml['enderEmit/email'], {:align => :center, :valign => :center, border: 0}

      @pdf.ibox 3.92, 3.08, 9.71, 2.54

      @pdf.ibox 0.60, 3.08, 9.71, 2.54, '', "DANFE", {:size => 12, :align => :center, :border => 0, :style => :bold}
      @pdf.ibox 1.20, 3.08, 9.71, 3.14, '', "DOCUMENTO AUXILIAR DA NOTA FISCAL ELETRÔNICA", {:size => 8, :align => :center, :border => 0}
      @pdf.ibox 0.60, 3.08, 9.71, 4.34, '', "#{@xml['ide/tpNF']} - " + (@xml['ide/tpNF'] == '0' ? 'ENTRADA' : 'SAÍDA'), {:size => 8, :align => :center, :border => 0}

      @pdf.ibox 1.00, 3.08, 9.71, 4.94, '',
        "N°. " + @xml['ide/nNF'] + "\n" +
      ...
      if @xml['chNFe'].present?
        ...
        @pdf.ibox 2.20, 8.02, 12.79, 2.54
        @pdf.ibox 1.50, 8.00, 13.4010, 4.44, '', "MODELO", {:size => 14, :align => :center, :border => 0, :style => :bold}
        ...
        @produtos_box = @pdf.itable 6.37, 21.50, 0.25, 18.17,
          @xml.collect('xmlns', 'det')  { |det|
            [
              ...
              Helper.numerify(det.css('prod/qCom').text, 4),
